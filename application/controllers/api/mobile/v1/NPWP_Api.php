...
class NPWP_Api extends REST_Controller {
    ...
    {
        $response = $this->NPWP_Model->pagination($page, $per_page)->result();
        ...
        $path .= 'api/mobile/v1/npwp/pagination/page';
    ...
    public function pagination_post($page, $per_page)
    {
        try {
            ...
            $response = $this->NPWP_Model->pagination($page, $per_page, $search)->result();
            $total_rows = $this->NPWP_Model->count_pagination($search);
            $total_page = 1;
            $last_page = 1;
            $path = ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == "on") ? "https" : "http");
            $path .= "://" . $_SERVER['HTTP_HOST'];
