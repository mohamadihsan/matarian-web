...
$(document).ready( function () {
    ...
    actionExportToPdf ? buttonAction.push({ // button export to pdf
        ...
        customize: function (doc) {
            ...
            doc.content[1].table.widths = [ '15%', '20%', '15%', '10%', '10%', '10%', '19%', '0%' ];
    ...
    $('#katapandaTable tbody').on( 'click', 'tr', function () {
        ...
        var ids = $.map(table.rows(this).data(), async function (item) {
            ...
            item.jenis_kelamin == 'laki-laki' ? $('#jenisKelamin').prop('checked',true) : $('#jenisKelamin2').prop('checked',true);
            $('#jalan').val(item.alamat);
            $('#rt').val(item.rt);
            $('#rw').val(item.rw);
            $('#golonganDarah').val(item.golongan_darah).trigger('change');
            item.status_perkawinan == 'kawin' ? $('#statusPerkawinan').prop('checked',true) : $('#statusPerkawinan2').prop('checked',true);
