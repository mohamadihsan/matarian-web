...
<div class="container-fluid" id="container-wrapper">
    ...
    <script>
        $(document).ready(function() {
            ...
            getLangganan();
            getSalesAR();
            // actionExportToPdf ? buttonExportPDFCustom() : '';
            ...
            actionExportToPdf ? buttonAction.push({ // button export to pdf
                ...
                action: function(e, dt, node, config) {
                    ...
                    let kodeLangganan = $('#kodeLangganan').val() == '' || $('#kodeLangganan').val() == 'ALL' ? 'null' : $('#kodeLangganan').val();
                    let salesAR = $('#salesAR').val() == '' || $('#salesAR').val() == 'ALL' ? $('#salesAR').val() == '' ? 'null' : $('#salesAR').val() : $('#salesAR').val();
                    window.open(`<?= site_url() ?>accardat/tagihan/klik2/${moment(fromDate, 'DD-MM-YYYY').format('YYYY-MM-DD')}/${moment(endDate, 'DD-MM-YYYY').format('YYYY-MM-DD')}/${kodeLangganan}/${salesAR}/export`, '_blank');
                }
            }) : '';
            ...
            $('#kodeLangganan').change(function() {
                buttonExportPDFCustom();
            })

            $('#salesAR').change(function() {
                buttonExportPDFCustom();
            })
        ...
        function buttonExportPDFCustom() {
            let fromDate = $('#fromDate').val();
            let endDate = $('#endDate').val();
            let kodeLangganan = $('#kodeLangganan').val() == '' || $('#kodeLangganan').val() == 'ALL' ? 'null' : $('#kodeLangganan').val();
            let salesAR = $('#salesAR').val() == '' || $('#salesAR').val() == 'ALL' ? 'null' : $('#salesAR').val();
            // $('#btnExportPDFCustom').html(`<a href="<?= site_url() ?>accardat/tagihan/klik2/${moment(fromDate, 'DD-MM-YYYY').format('YYYY-MM-DD')}/${moment(endDate, 'DD-MM-YYYY').format('YYYY-MM-DD')}/${kodeLangganan}/${salesAR}/export" target="_blank" class="btn btn-sm btn-outline-info"><i class="fas fa-fas fa-file-pdf"></i> Export PDF Custom</a>`);
        }
    </script>