...
class Dashboard_Model extends CI_Model
{
    ...
    public function count_tagihan($user, $sales_ar)
    {
        ...
        if ($sales_ar != null || $sales_ar != '') {
            $this->db->where_in('sales_ar', $sales_ar);
        }
    ...
    {
        if ($year && $month) {
            $this->db->where("DATE_FORMAT(ta.tanggal_bon,'%Y-%m')", sprintf('%04d-%02d', $year, $month));
        } elseif ($year) {
            $this->db->where("DATE_FORMAT(ta.tanggal_bon,'%Y')", $year);
        } elseif ($month) {
            $this->db->where("DATE_FORMAT(ta.tanggal_bon,'%m')", sprintf('%02d', $month));
        }
        ...
        $this->db->where('dat.sales_ar IS NOT NULL', null, false);
        $this->db->select('ta.kode_langganan, lgn.nama_toko, COUNT(ta.kode_langganan) as total_transaction, SUM(ta.banyak_barang*ta.harga_barang) as total_purchase');
        ...
        $this->db->group_by(array('ta.kode_langganan', 'lgn.nama_toko'));
    ...
    public function best_seller($year, $month, $limit, $sales_ar)
    {
        ...
        $this->db->join('tbl_accardat dat', 'dat.nomor_nota = ta.nomor_bon', 'left');
        $this->db->group_by(array('ta.kode_barang', 'br.nama_barang'));
    ...
    {
        $this->db->distinct();
        $this->db->where('DATE(tbl_login.login_at)', date('Y-m-d'));
        $this->db->where('tbl_login.logout_at IS NULL', null, false);
        ...
        $this->db->group_by('tbl_user.fullname');
        $this->db->group_by('tbl_user.email');
        $this->db->group_by('tbl_login.ip_address_var');
        $this->db->group_by('tbl_user_group.user_group_name');
        $this->db->group_by('tbl_login.logout_at');
        ...
        $this->db->order_by('MAX(tbl_login.login_at)', 'desc');
